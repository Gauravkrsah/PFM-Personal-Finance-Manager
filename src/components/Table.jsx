  ...
  return (
    <div className="card p-6 border-0 shadow-none bg-transparent sm:bg-white sm:shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:border sm:border-paper-200/60">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h2 className="font-semibold text-xl tracking-tight">Expenses</h2>
          <p className="text-gray-500 text-xs mt-0.5 sm:block hidden">Manage and track your daily spending</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <DateRangePicker value={dateRange} onChange={setDateRange} className="flex-1 sm:w-48" />
          <button onClick={fetchExpenses} disabled={loading} className="px-3 py-2 text-xs bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-gray-700">
            {loading ? '...' : 'Refresh'}
          </button>
        </div>
      </div>
      ...
      <div className="flex md:grid md:grid-cols-3 gap-3 mb-6 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
        <div className="bg-red-50 p-3 md:p-4 rounded-xl border border-red-100 shadow-sm relative overflow-hidden min-w-[170px] md:min-w-0 flex-shrink-0 md:flex-shrink">
          <div className="relative z-10">
            <div className="text-[10px] md:text-xs font-semibold text-red-800 uppercase tracking-wide mb-1">Total Expenses</div>
            <div className="text-lg md:text-2xl font-bold text-red-900">Rs.{filteredData.reduce((sum, exp) => sum + (exp.amount || 0), 0).toLocaleString()}</div>
            <div className="text-[10px] md:text-xs text-red-700 mt-1">{filteredData.length} transactions</div>
          </div>
        ...
        <div className="bg-orange-50 p-3 md:p-4 rounded-xl border border-orange-100 shadow-sm relative overflow-hidden min-w-[170px] md:min-w-0 flex-shrink-0 md:flex-shrink">
          <div className="relative z-10">
            <div className="text-[10px] md:text-xs font-semibold text-orange-800 uppercase tracking-wide mb-1">This Period</div>
            ...
            <div className="text-[10px] md:text-xs text-orange-700 mt-1">Filtered range</div>
          </div>
          <div className="absolute right-0 bottom-0 opacity-10 transform translate-x-1/4 translate-y-1/4">
            <svg width="70" height="70" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05 1.18 1.91 2.53 1.91 1.29 0 2.13-.73 2.13-1.65 0-1.22-1.28-1.57-3.04-1.93-2.26-.47-4.14-1.29-4.14-3.56 0-1.84 1.37-2.92 3.82-3.32V4h2.67v1.89c1.4.31 2.54 1.25 2.76 3.01h-2c-.17-.9-1.07-1.54-1.99-1.54-1.12 0-1.77.67-1.77 1.48 0 1.13 1.27 1.47 2.89 1.83 2.45.54 4.29 1.35 4.29 3.65 0 1.96-1.56 3.12-3.57 3.48z" /></svg>
          </div>
        ...
        <div className="bg-amber-50 p-3 md:p-4 rounded-xl border border-amber-100 shadow-sm relative overflow-hidden min-w-[170px] md:min-w-0 flex-shrink-0 md:flex-shrink">
          <div className="relative z-10">
            <div className="text-[10px] md:text-xs font-semibold text-amber-800 uppercase tracking-wide mb-1">Avg/Transaction</div>
            ...
            <div className="text-[10px] md:text-xs text-amber-700 mt-1">Average amount</div>
          </div>
          <div className="absolute right-0 bottom-0 opacity-10 transform translate-x-1/4 translate-y-1/4">
            <svg width="70" height="70" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05 1.18 1.91 2.53 1.91 1.29 0 2.13-.73 2.13-1.65 0-1.22-1.28-1.57-3.04-1.93-2.26-.47-4.14-1.29-4.14-3.56 0-1.84 1.37-2.92 3.82-3.32V4h2.67v1.89c1.4.31 2.54 1.25 2.76 3.01h-2c-.17-.9-1.07-1.54-1.99-1.54-1.12 0-1.77.67-1.77 1.48 0 1.13 1.27 1.47 2.89 1.83 2.45.54 4.29 1.35 4.29 3.65 0 1.96-1.56 3.12-3.57 3.48z" /></svg>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-black/5 outline-none transition-all placeholder:text-gray-400"
          />
        ...
        <div className="relative sm:w-48">
          <select
            ...
            className="w-full pl-4 pr-10 py-2 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-black/5 outline-none font-medium text-gray-700 appearance-none cursor-pointer hover:border-gray-400 transition-colors"
          >
            {categories.map(cat => (
        ...
        <table className="w-full text-sm text-left min-w-[800px]">
          <thead>
            <tr className="bg-paper-50/50 border-b border-paper-100">
              ...
              <th className="p-4 font-semibold text-gray-900 text-xs uppercase tracking-wider">Item</th>
              <th className="p-4 font-semibold text-gray-900 text-xs uppercase tracking-wider">Category</th>
          ...
          <tbody className="divide-y divide-paper-100">
            {loading ? (
              ...
              filteredData.map((expense) => (
                <tr key={expense.id} className="hover:bg-paper-50/50 transition-colors group">
                  {editingId === expense.id ? (
                    ...
                    // View Mode
                    <>
                      <td className="p-4 text-gray-600 text-sm whitespace-nowrap">{new Date(expense.date).toLocaleDateString()}</td>
                      <td className="p-4 font-medium text-gray-900 text-sm">{expense.item}</td>
                      <td className="p-4">
                        <span className="px-3 py-1 bg-paper-100 text-gray-700 rounded-full text-xs font-medium border border-paper-200">
                          {expense.category}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-gray-700 text-sm max-w-[200px] truncate" title={expense.remarks || ''}>
                        {expense.remarks || '-'}
                      </td>
          ...
          <tfoot>
            <tr className="bg-paper-50 border-t border-paper-100 font-medium">
              <td className="p-4 text-gray-900" colSpan="3">Total Expenses</td>
              <td className="p-4 text-gray-900 font-bold">Rs.{filteredData.reduce((sum, exp) => sum + (exp.amount || 0), 0).toLocaleString()}</td>
